...
- `sum_common.py` содержит разбиение диапазона, формулу суммы арифметической прогрессии, расчет ожидаемого значения и вывод результата;
...
Сумма внутри каждого диапазона считается по формуле арифметической прогрессии:
...
Такой подход выбран потому, что прямой перебор до `10_000_000_000_000` занял бы слишком много времени и не был бы практичным для демонстрации работы потоков, процессов и задач `asyncio`.
...
Особенность подхода: потоки удобны для задач ожидания ввода-вывода, но для CPU-bound вычислений в Python ограничены GIL. В данной реализации расчет быстрый, потому что используется формула.
...
Асинхронная версия использует `aiohttp` для HTTP-запросов и `asyncio.gather()` для одновременного запуска задач. Запись в БД выполняется через `asyncio.to_thread()`, потому что операции SQLModel/SQLAlchemy синхронные.
...
| --- | ---: | ---: | ---: |
| Сумма, `--limit 1000000 --workers 4` | 0.001848 s | 0.234405 s | 0.002962 s |
| Парсер, URL по умолчанию | 0.994758 s | 1.537587 s | 0.953795 s |

По результатам видно:
- для задачи суммы все варианты работают быстро, потому что вместо прямого цикла используется формула;
- `multiprocessing` оказался медленнее на малом демонстрационном запуске из-за накладных расходов на создание процессов;
...
В ходе лабораторной работы были реализованы и сравнены три модели конкурентного выполнения в Python: потоки, процессы и асинхронные задачи. Для вычислительной задачи наиболее подходящим подходом в общем случае является `multiprocessing`, так как он позволяет использовать несколько ядер процессора, однако на демонстрационном расчете накладные расходы процессов оказались выше полезной работы. Для сетевого парсинга лучше подходят `threading` и `asyncio`, поскольку основное время тратится на ожидание HTTP-ответов. Асинхронная реализация с `aiohttp` показала лучший результат среди вариантов парсера и является наиболее удобной для масштабирования на большое количество URL.
